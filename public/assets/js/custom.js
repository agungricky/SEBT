...
$(document).ready(function () {
    ...
    // ======================= Define Firebase Realtime & Update Firebase ======================= //
    var firebaseConfig = {
    ...
    function renderCompositeFormula() {
        ...
        if (maxKananGlobal.length === 8 && maxKiriGlobal.length === 8) {
            ...
            let sumKanan = maxKananGlobal.map(v => v.toFixed(2)).join(" + ");
            let sumKiri = maxKiriGlobal.map(v => v.toFixed(2)).join(" + ");

            formulaKiri = `<span class="formula">\\( CS_L = \\frac{(${sumKiri}) \\times 100}{8 \\times ${panjang_kiri}} \\)</span>`;
            formulaKanan = `<span class="formula">\\( CS_R = \\frac{(${sumKanan}) \\times 100}{8 \\times ${panjang_kanan}} \\)</span>`;
            ...
            function kategori(nilai) {
                ...
                if (nilai >= 90) return { text: "Baik", color: "primary" };
                if (nilai >= 85) return { text: "Cukup", color: "warning" };
                return { text: "Kurang", color: "danger" };
            }
            ...
            let sisiDominan = maxKiriGlobal[0] > maxKananGlobal[0] ? "Kiri lebih baik" :
                maxKananGlobal[0] > maxKiriGlobal[0] ? "Kanan lebih baik" : "Seimbang";
            let statusCidera = selisih > 4 ? "Cidera" : "Tidak Cidera";

            // Tampilkan hasil
            if (hasil) {
                $('#hasilCSL').text(csl.toFixed(2) + '%');
                $('#kategoriCSL').html(`<span class="text-${kategoriKiri.color} fw-semibold">${kategoriKiri.text}</span>`);

                $('#hasilCSR').text(csr.toFixed(2) + '%');
                $('#kategoriCSR').html(`<span class="text-${kategoriKanan.color} fw-semibold">${kategoriKanan.text}</span>`);
                ...
                <div class="bg-light rounded-3 p-3">
                    <div class="row text-center align-items-center">
                        ...
                        <div class="col-12 col-sm-3 border-start">
                            <div class="text-muted small">Selisih</div>
                            <div class="fs-5 fw-bold">${selisih.toFixed(2)}</div>
                        </div>
                        <div class="col-12 col-sm-3 mb-2 mb-sm-0 border-start">
                            <div class="text-muted small">Keterangan</div>
                            <div class="fw-bold">${sisiDominan}</div>
                        </div>
                        <div class="col-12 col-sm-3 mb-2 mb-sm-0 border-start">
                            <div class="text-muted small">Status</div>
                            <div class="fw-semibold">${statusCidera}</div>
                        </div>
                    </div>
                </div>
            `;
            ...
            // Rumus default ketika data belum lengkap
            formulaKiri = `<span class="formula">\\( \\small CS_L = \\frac{(A_L + AM_L + M_L + PM_L + P_L + PL_L + L_L + AL_L) \\times 100}{8 \\times PT} \\)</span>`;
            formulaKanan = `<span class="formula">\\( \\small CS_R = \\frac{(A_R + AM_R + M_R + PM_R + P_R + PL_R + L_R + AL_R) \\times 100}{8 \\times PT} \\)</span>`;
    ...
    $('#submitData').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                nilai_kanan = new Array(24).fill(0);
                nilai_kiri = new Array(24).fill(0);
                normalisasi = new Array(16).fill(0);
                maxKananGlobal = [];
                maxKiriGlobal = [];
                hasilNormalisasi = [];
                formData = {};
